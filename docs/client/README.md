...
it. The device uploads the member and the encrypted manifest through the storage client and then
publishes the descriptor through the manifest client, signed by the writer. Both clients present the
account token for `backup.write`, and the sign-in that turns recovery-enabled backup on is the only
