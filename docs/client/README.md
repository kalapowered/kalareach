...
call, and a relay lease request may have issued and installed a lease. Everything `services::signed`
carries is safe to send again: each method is an idempotent read or is keyed, and each service
answers a repeat of a keyed write from what it decided the first time. So are a revocation, the
identity and usage reads, the voice terms and closing a voice call.
