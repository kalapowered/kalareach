...
- The plan records a reservation as asked for before `upload.begin` leaves, and only the host's
  result or its refusal clears that. A lost reply, an answer saying the outcome is unknown, a
  receipt with no result and a call abandoned part way all leave the plan refusing to reserve
  again. Nobody knows whether the host holds a transfer for it, so uploading the same file a second
  time is the person's decision.
- A lane reads whatever the host has already sent on it before each call, so a lane that sat idle
  sends under the window the host renewed meanwhile rather than one that expired.
