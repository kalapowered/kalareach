...
writes nothing until a read finds the write's bytes or `end_lost_write` fences its identity. The
record also outlives an answer, because `complete_migration` needs it: a migration whose destination
