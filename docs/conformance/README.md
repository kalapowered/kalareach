...
with `--nocapture`, keeps its script's command. Before a step runs, its tests are built, and every
test binary the build made is asked for its list and then has to be run and read: a binary that
does not list, one the log never ran, and a log that cannot be read are each the step's error. A target whose manifest
gives it a harness of its own (`harness = false`) is a program that prints neither a list nor
verdicts: it is not asked for a list, it is run with the step, its exit status is the step's, and a
...
| In test code, a comment on a function | Every test of the same target whose body calls that function, the call followed step by step: module definitions, `crate`, `self` and `super`, a `use` that brings an item in by its own name, and a glob exactly one module answers for. A renaming `use`, a name or glob the calling body brings in for itself, and anything else is not followed, and a call that is not followed keys nothing, so the function's identifiers stay references rather than become a key the compiler would not make |
