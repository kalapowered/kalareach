 ...
 * liveness limit against those measurements: about twice the slowest case seen, and still short
 * enough to leave the whole of a test inside the project's own thirty-second limit. A machine can
 * always be slow enough to pass any fixed figure, so this does not prove a page has stopped; it is
 * the point past which waiting any longer is worth less than being told what the wait was for.
 */
