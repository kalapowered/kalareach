...
Recognises Codex CLI and names the session after it.

## What it does

The package carries one match rule and one document node. KalaReach matches the `codex` executable,
labels the session, and runs Codex through its ordinary terminal path, exactly as it would run any
other program.
...
KalaReach read Codex's own protocol: the framing, the request identifiers, the response
