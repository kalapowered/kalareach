...
Recognises OpenCode and names the session after it.

## What it does

The package carries one match rule and one document node. KalaReach matches the `opencode` executable,
labels the session, and runs OpenCode through its ordinary terminal path, exactly as it would run any
other program.
...
KalaReach read OpenCode's own protocol: the framing, the request identifiers, the response
