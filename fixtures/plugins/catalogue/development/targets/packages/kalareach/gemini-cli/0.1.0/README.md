...
Recognises Gemini CLI and names the session after it.

## What it does

The package carries one match rule and one document node. KalaReach matches the `gemini` executable,
labels the session, and runs Gemini CLI through its ordinary terminal path, exactly as it would run any
other program.
...
KalaReach read Gemini CLI's own protocol: the framing, the request identifiers, the response
