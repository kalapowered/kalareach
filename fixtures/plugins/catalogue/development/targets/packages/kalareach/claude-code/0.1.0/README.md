...
Recognises Claude Code and names the session after it.

## What it does

The package carries one match rule and one document node. KalaReach matches the `claude` executable,
labels the session, and runs Claude Code through its ordinary terminal path, exactly as it would run any
other program.
...
KalaReach read Claude Code's own protocol: the framing, the request identifiers, the response
