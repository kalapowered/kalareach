...
//! exists, and then once more. It then prints `<bound> <average> <allowance> <times>`: the first
//! three rounded up to two decimal places, the most processors' worth of processor time the machine
//! can have spent on anything but the run in any five seconds from the first reading to the last,
//! the same over all of that time, and how much of the bound the counts' allowances for rounding
//! and trailing make up; then `threads` where the readings took the run's time thread by thread,
//! or `processes` where they took each process's time whole. Where the readings cannot show that,
//! it prints `unread: <why>` instead. The run is process `--run` and every process descended from
//! it; a run whose process has gone is unread, so a watcher whose run was stopped stops too.
...
fn watch(options: &Options) -> Result<(Summary, Times), String> {
    let mut machine = Machine::open()?;
    let mut tally = Tally::new(options.run);
    let reading = machine.reading(|rows| tally.candidates(rows))?;
    tally.add(&reading)?;
    ...
    let reading = machine.reading(|rows| tally.candidates(rows))?;
    tally.add(&reading)?;
    Ok((tally.summary(WINDOW)?, machine.times()))
}
