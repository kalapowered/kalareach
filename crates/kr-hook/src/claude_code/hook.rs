...
mod tests {
    ...
    async fn a_refusal_that_closes_before_the_observation_is_written_is_reported_as_one() {
        ...
        let paths = crate::registration::Paths {
            ...
            credential: host.root().join("credential"),
        };
        std::fs::write(&paths.credential, "0".repeat(64)).expect("the credential");
        std::fs::set_permissions(&paths.credential, std::fs::Permissions::from_mode(0o600))
