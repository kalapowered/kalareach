...
fn concurrency_limits_are_enforced_per_workflow() {
    ...
    assert!(controller.admit_run(wf1, grant, 1000, max_conc, None).is_ok());
    assert!(controller.admit_run(wf1, grant, 1000, max_conc, None).is_ok());

    // 3rd concurrent run for wf1 is rejected
    let err = controller.admit_run(wf1, grant, 1000, max_conc, None).unwrap_err();
    assert!(err.to_string().contains("concurrency limit"));

    // wf2 is unaffected by wf1's limit
    assert!(controller.admit_run(wf2, grant, 1000, max_conc, None).is_ok());
...
fn host_wide_rate_limits_throttle_excessive_traffic() {
    ...
    let err = controller.admit_run(wf1, grant, 1000, None, None).unwrap_err();
...
fn per_grant_rate_limits_isolate_tenants() {
    ...
    // Exhaust grant_a
    assert!(controller.admit_run(wf, grant_a, 1000, max_conc, None).is_ok());
    assert!(controller.admit_run(wf, grant_a, 1000, max_conc, None).is_ok());
    let err_a = controller.admit_run(wf, grant_a, 1000, max_conc, None).unwrap_err();
    assert!(err_a.to_string().contains("grant"));

    // grant_b has separate quota and succeeds
    assert!(controller.admit_run(wf, grant_b, 1000, max_conc, None).is_ok());
    assert!(controller.admit_run(wf, grant_b, 1000, max_conc, None).is_ok());
    let err_b = controller.admit_run(wf, grant_b, 1000, max_conc, None).unwrap_err();
