                ...
                match tokio::time::timeout(remaining, self.client.recv()).await {
                        ...
                        if notification.event_type.as_str() == SESSION_CLOSED_EVENT =>
                    {
                        return match notification.payload.to_typed::<ClosureRecord>() {
                            Ok(record) => Found::Closure { record, output },
...
/// The byte the stalled-attachment session writes, and how many of it.
const GENERATED: u8 = b'x';
const GENERATED_COUNT: usize = 2_000_000;

/// What a client that reads its stream to the end finds there.
#[derive(Debug)]
enum Found {
    /// The closure, and how many generated bytes of output came before it.
    Closure {
        record: ClosureRecord,
        output: usize,
    },
    /// The end of the connection with no closure before it: when this test saw it, why, and how
    /// many generated bytes of output came first.
    Ended {
        at: SystemTime,
        error: String,
        output: usize,
...
/// How far into that wait a client that stopped reading may start reading again for what it is
/// then sent to count.
///
/// One that starts well inside the wait has time to read what it was owed; one that starts near
/// its end is racing the bound, and shows nothing either way.
...
///
/// A worker starts its wait a moment after it writes the record, and a sample that finds it gone
/// comes a moment after it went.
const BOUND_SLACK: Duration = Duration::from_millis(500);

/// What a worker may take beyond its bound to end once it has stopped waiting.
...
/// depends on: the closure seen too late in the worker's wait, or a watch that could not place the
/// worker's end. Such a session shows nothing either way. A check that what was seen contradicts
/// fails here.
///
...
fn stalled_attachments(host: &Host) -> (SessionId, Result<(), String>) {
    ...
    // The client that never reads again is the one that types, so the only reading either client
    // does before the closure is that one waiting for the answer to what it typed. The other has
    // been sent everything the session wrote and has read none of it.
    ...
    never.type_bytes(
        host,
        format!("head -c {GENERATED_COUNT} /dev/zero | tr '\\0' x; exit 0\r").as_bytes(),
    );
    let closed = host.wait_until_closed(&created.session_id.to_string());
    ...
    if since(seen) > RESUME_WINDOW {
        missed.push(format!(
            "the closure was seen {:?} into the worker's wait, too near its bound",
            since(seen)
        ));
    } else {
        ...
        if !still_there {
            assert!(
                ...
                "the worker had gone {:?} after its session closed, while it still owed two \
                 attachments how",
                since(looked)
            );
            missed.push(format!(
                "the worker was first looked for {:?} into its wait, at its bound",
                since(looked)
            ));
        } else {
            match resuming.read_to_the_end(host) {
                Found::Closure { record, output } => {
                    ...
                    assert!(
                        ...
                        "the closure came after every byte of output the client was owed, and \
                         only {output} of the {GENERATED_COUNT} came first"
                    );
                }
                Found::Ended { at, error, output } => {
                    assert!(
                        since(at) + BOUND_SLACK >= NOTICE_BOUND,
                        "the connection of the client that read again ended {:?} after the \
                         closure, before the worker's bound, after {output} bytes and no \
                         closure: {error}",
                        since(at)
                    );
                    missed.push(format!(
                        "the client that read again was still reading {:?} into the worker's \
                         wait, at its bound",
                        since(at)
                    ));
                }
                Found::Undecodable(error) => {
                    panic!(
                        "the client that read again was sent a closure that did not decode: {error}"
                    )
                }
                Found::Silent => panic!(
    ...
    let seen_end = watch.finish()[0];
    let (at_least, at_most) = (since(seen_end.running_at), since(seen_end.gone_at));
    assert!(
        at_most + BOUND_SLACK >= NOTICE_BOUND,
        "the worker had gone {at_most:?} after the closure, before its bound, while a client still \
         owed the closure had not read it"
    );
    assert!(
        at_least <= NOTICE_BOUND + EXIT_ALLOWANCE,
        "the worker was still running {at_least:?} after the closure, past its bound"
    );
    if at_least + BOUND_SLACK < NOTICE_BOUND || at_most > NOTICE_BOUND + EXIT_ALLOWANCE {
        missed.push(format!(
            "the worker's end lay between {at_least:?} and {at_most:?} after the closure, and the \
             watch could not hold it to the bound"
        ));
    }
    assert_eq!(worker_exit_status(&created.worker), 0);
    ...
    match never.read_to_the_end(host) {
        Found::Ended { .. } => {}
        Found::Closure { record, output } => panic!(
            "the client that never read again was sent the closure after {output} bytes: {record:?}"
        ),
