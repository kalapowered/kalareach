...
impl Stalling {
    ...
    ///
    /// The output is read as the one stream it is, and what is kept of it is the longest run of
    /// the generated byte: a byte of it lost, or one sent twice, changes that run.
                ...
                match tokio::time::timeout(remaining, self.client.recv()).await {
                        ...
                        if notification.event_type.as_str() == SESSION_CLOSED_EVENT =>
                    {
                        return match notification.payload.to_typed::<ClosureRecord>() {
                            Ok(record) => Found::Closure { record, longest },
                        ...
                        if notification.event_type.as_str() == "session.output" =>
                    {
                        ...
                        for byte in event.bytes.as_slice() {
...
#[derive(Debug)]
enum Found {
    /// The closure, and the longest run of the generated byte that came before it.
    Closure {
...
/// What a worker may take beyond its bound to end, counted from when this test saw its session
/// closed.
...
/// fails here.
///
/// Two moments bound the worker's wait. It cannot begin before the time the record carries, so
/// nothing it owes can end it sooner than that time and the bound. It has begun by the time this
/// test sees the session closed, so it has to have ended by that moment, the bound and an
/// allowance. The record's time is taken before the record is written durably, which on a busy
/// machine can be seconds before the session reads as closed: each end is measured from the
/// moment that is sound for it.
///
...
fn stalled_attachments(host: &Host) -> (SessionId, Result<(), String>) {
    ...
    never.type_bytes(
        host,
        format!(
            "read -r _ < \"$PWD\"/flood; head -c {GENERATED_COUNT} /dev/zero | tr '\\0' '\\101'; \
             exit 0\r"
        )
        .as_bytes(),
    );
    ...
    let after_seen = |moment: SystemTime| moment.duration_since(seen).unwrap_or_default();
    ...
    assert!(
        after_seen(end.running_at) <= NOTICE_BOUND + EXIT_ALLOWANCE,
        "the worker was still running {:?} after its session read as closed, past its bound",
        after_seen(end.running_at)
    );
    if since(end.running_at) + BOUND_SLACK < NOTICE_BOUND
        || after_seen(end.gone_at) > NOTICE_BOUND + EXIT_ALLOWANCE
    {
        missed.push(format!(
    ...
    match never.read_to_the_end(host) {
        Found::Ended { .. } => {}
        Found::Closure { record, longest } => panic!(
