...
#[cfg(target_os = "macos")]
fn serving_in(listing: &str, image: &Path) -> Vec<String> {
...
fn the_processes_serving_an_image_are_named_from_hdiutil_info() {
    ...
    assert_eq!(
        serving_in(&listing, Path::new("/private/tmp/served.dmg")),
        [own.clone(), "303".to_owned()],
        "both attachments of the image, and not the other image's"
    );
    assert!(serving_in(&listing, Path::new("/private/tmp/absent.dmg")).is_empty());
