...
pub fn materialise(
    ...
    if let Err(error) = recorded {
        ...
        let _ = parent.sync(kr_flush::NameKind::Directory);
        return Err(error);
    }
...
fn walk(
    ...
    for entry in entries {
        ...
        // A path the input version **deleted** that this materialisation holds again is a path put
        // back, not a file out of nowhere: what the base held for it travels with the deletion, so
        // the derived version records the same base and a later revert still has somewhere to go.
        let restored = previous
            ...
            .then(|| {
                original
                    .deletions
                    .iter()
                    .find(|deleted| deleted.path == path)
            })
            .flatten();
        ...
        found.paths.push(CapturedPath {
            ...
            class: match previous {
                ...
                // A path the input version deleted and this reading holds is tracked work again,
                // not an untracked addition.
                None if restored.is_some() => PathClass::DirtyFile,
                None => PathClass::UntrackedFile,
            },
            change: ChangeKind::Present,
            base_object_id: previous.map_or_else(
                || Nullable(restored.and_then(|deleted| deleted.base_object_id.clone())),
                |entry| entry.base_object_id.clone(),
            ),
            base_mode: previous.map_or_else(
                || Nullable(restored.and_then(|deleted| deleted.base_mode.clone())),
                |entry| entry.base_mode.clone(),
            ),
            path,
        });
    }
