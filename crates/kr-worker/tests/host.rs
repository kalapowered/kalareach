...
impl RunningDaemon {
    ...
    async fn stop(self) {
        ...
        // The lock is released when the last reference goes. Giving the runtime a moment to drop
        // the aborted tasks' own references is what makes the next start deterministic.
