...
fn kr_req_12_13_an_identifier_keeps_its_json_type_and_a_request_resolves_nothing() {
    ...
    for not_a_failure in [
        ...
        // A code that parsed as a floating number has already been rounded, so this host cannot
        // tell 1.00000000000000001 from the 1 it became, and does not resolve a resource on a
        // code the upstream may never have sent. The cost is that an integer spelled with a point
        // or an exponent is refused too.
        ...
        r#"{"id":"11","error":{"code":-3.2601e4,"message":"no such method"}}"#,
    ] {
        assert!(
            broker
                .native_answer(
