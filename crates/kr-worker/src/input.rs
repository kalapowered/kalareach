...
/// The sequence an application sends to turn bracketed paste on.
pub const BRACKETED_PASTE_ON: &[u8] = b"\x1b[?2004h";

/// The sequence an application sends to turn bracketed paste off.
pub const BRACKETED_PASTE_OFF: &[u8] = b"\x1b[?2004l";

/// Returns the bracketed-paste mode an output batch last set, when it set one.
///
/// The application is the only thing that decides whether bracketed paste is on, and it says so on
/// its output stream. The **last** occurrence in a batch wins, because that is the one in force
/// when the batch ends.
#[must_use]
pub fn bracketed_paste_mode(output: &[u8]) -> Option<bool> {
    let on = last_occurrence(output, BRACKETED_PASTE_ON);
    let off = last_occurrence(output, BRACKETED_PASTE_OFF);
    match (on, off) {
        (None, None) => None,
...
mod mode_tests {
    ...
    #[test]
    fn an_application_turning_bracketed_paste_on_is_noticed() {
        assert_eq!(bracketed_paste_mode(b"\x1b[?2004h"), Some(true));
        assert_eq!(bracketed_paste_mode(b"prompt \x1b[?2004h"), Some(true));
    }

    #[test]
    fn an_application_turning_it_off_is_noticed() {
        assert_eq!(bracketed_paste_mode(b"\x1b[?2004l"), Some(false));
    }

    #[test]
    fn the_last_change_in_a_batch_is_the_one_in_force() {
        assert_eq!(bracketed_paste_mode(b"\x1b[?2004h\x1b[?2004l"), Some(false));
        assert_eq!(bracketed_paste_mode(b"\x1b[?2004l\x1b[?2004h"), Some(true));
    }

    #[test]
    fn output_that_changes_nothing_reports_nothing() {
        assert_eq!(bracketed_paste_mode(b"ordinary output"), None);
        assert_eq!(bracketed_paste_mode(b""), None);
        assert_eq!(bracketed_paste_mode(b"\x1b[?1049h"), None);
    }

    #[test]
    fn a_held_prefix_that_ran_out_of_time_is_forwarded_before_new_bytes() {
        let mut framer = PasteFramer::new();
        framer.set_bracketed_paste(true);
        let start = std::time::Instant::now();
        let held = framer.push(b"\x1b[200", start);
        assert!(held.forward.is_empty(), "the prefix is held");
        // The rest of the delimiter arrives after the deadline. It is no longer a delimiter.
        let late = framer.push(b"~", start + RECOGNISER_DEADLINE + Duration::from_millis(1));
        assert_eq!(
            late.forward, b"\x1b[200~",
            "the bytes are forwarded as themselves, in order"
        );
        assert!(
            !late.paste_started,
            "two bytes twenty-six milliseconds apart are not a delimiter"
        );
    }
}
