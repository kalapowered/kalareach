...
/// comes first because the rows are converted in order against a running total of the queue, so
/// what a queue that runs out gives up is the part furthest from what the person is looking at.
/// Rows the window already covers are not read twice.
///
